#!/usr/bin/env python3
"""
Phase-0 smoke test for the CoreWeave Sandbox (cwsandbox) provider.

Talks directly to the CW Sandbox Gateway HTTP API to validate the
...
to reach the Gateway, while curl uses the OS's. Zero pip dependencies.
"""
...
class Client:
    """CW Sandbox Gateway client over curl (Bearer auth, JSON)."""
    ...
    def exec(self, sandbox_id: str, command: str) -> tuple[int, str, str]:
        ...
        # proto3 JSON omits exit_code when 0.
